        ...
        <div className="container px-5 py-24 mx-auto flex flex-wrap flex-col">
          ...
          <div className="flex mx-auto mb-10 justify-center">
            <ScrollArea className="w-80 lg:w-2/3 whitespace-nowrap rounded-md">
              {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
                <a 
                  ...
                  ${activeProjectGroup === each.title ? 
                    "bg-[#101820] dark:bg-[#F2AA4C] text-[#F2AA4C] dark:text-[#101820] border-[#F2AA4C] dark:border-[#101820] rounded-t" : 
              ...
              <ScrollBar orientation="horizontal" className="bg-[#101820] dark:bg-[#F2AA4C]" />
            </ScrollArea>
          </div>
          
          {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
            <React.Fragment key={index}>
              <div className={`flex flex-wrap text-center bg-[#101820] dark:bg-[#F2AA4C] text-[#F2AA4C] dark:text-[#101820] bg-opacity-60 px-4 py-6 rounded-lg ${activeProjectGroup === each.title ? "" : "hidden"}`}>
                ...
                {each?.projects?.map((each1, index1) => (
                  <React.Fragment key={index1}>
                    <div className="p-4 w-full md:w-1/3 text-left">
                      <div className="h-full bg-[#F2AA4C] dark:bg-[#101820] text-[#101820] dark:text-[#F2AA4C] rounded-lg overflow-hidden">
                          ...
                          <div className="flex items-center flex-wrap ">
                            ...
                            {each1?.links?.map((each2, index2) => (
                              each2?.title === "Visit Application" ? (
                                <span key={index2} className="mr-3 inline-flex items-center lg:ml-auto md:ml-0 ml-auto leading-none text-sm py-1 border-r-2 border-[#101820] dark:border-[#F2AA4C]">
                              ...
                              each2?.title === "Visit Admin" ? (
                                <span key={index2} className="mr-3 inline-flex items-center leading-none text-sm py-1 border-r-2 border-[#101820] dark:border-[#F2AA4C]">
                                  <Link to={each2?.url} target="_blank" rel="noreferrer" >
